local library = loadstring(
    game:HttpGet(
        'https://raw.githubusercontent.com/UnknownReality1337/venislib/refs/heads/main/main.lua'
    )
)()
...
local main = library:Load({
    Name = 'SeraphV3 - Fish It',
    SizeX = 600,
    SizeY = 600,
    Theme = 'Midnight',
    Extension = 'srph', -- config file extension
    Folder = 'SeraphUI', -- config folder name
})
...
local colorpickerpicker1 = colorpicker:ColorPicker({
    ...
    Callback = function(color)
        print(
            'Color picker 1 picker 1 is now: '
                .. string.format(
                    '%s, %s, %s, %s',
                    math.round(color.R * 255),
                    math.round(color.G * 255),
...
local colorpickerpicker2 = colorpicker:ColorPicker({
    ...
    Callback = function(color)
        print(
            'Color picker 1 picker 2 is now '
                .. string.format(
                    '%s, %s, %s, %s',
                    math.round(color.R * 255),
                    math.round(color.G * 255),
...
local save = configsection:Button({
    ...
    Callback = function()
        library:SaveConfig(
            library.flags['Config Dropdown'] or library.flags['Config Name']
        ) -- SaveConfig(library.flags["Config Name"], true) if you want universal configs
        configlist:Refresh(library:GetConfigs())
    end,
})

local keybindsection =
    configs:Section({ Name = 'UI Toggle Keybind', Side = 'Left' })
