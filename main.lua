...
local library = utility.table({theme = table.clone(themes.Default), folder = "seraphlib", extension = ".srph", flags = {}, open = true, keybind = Enum.KeyCode.F7, mousestate = services.InputService.MouseIconEnabled, cursor = nil, holder = nil, connections = {}}, true)
...
function library.createbox(box, text, callback, finishedcallback)
    ...
    hiddenBox.Text = ""
    hiddenBox.Parent = box -- or CoreGui/PlayerGui depending on your setup
...
function library.createkeybind(default, parent, blacklist, flag, callback, offset)
    ...
    local function set(newkey)
        ...
        if newkey ~= nil and not table.find(blacklist, newkey) then
            ...
            local sizeX = utility.textlength(text, Drawing.Fonts.System, 13).X
            ...
            local text = "[NONE]"
            local sizeX = utility.textlength("[NONE]", Drawing.Fonts.System, 13).X
    ...
    keybutton.MouseButton1Click:Connect(function()
        if not binding then
            local sizeX = utility.textlength("...", Drawing.Fonts.System, 13).X
...
function library:Watermark(str)
    local size = utility.textlength(str, Drawing.Fonts.System, 13).X
    ...
    function windowtypes:Tab(name)
        ...
        function tabtypes:Section(options)
            ...
            function sectiontypes:Separator(name)
                ...
                local sizeX = utility.textlength(name, Drawing.Fonts.System, 13).X
                ...
                function separatortypes:Set(str)
                    local sizeX = utility.textlength(str, Drawing.Fonts.System, 13).X
